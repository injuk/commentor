...
internal fun CreateCommentRequest.convert() = this.run {
...
internal fun CreateSubCommentRequest.convertWith(parentId: Long) = this.run {
    ga.injuk.commentor.application.port.dto.request.CreateCommentRequest(
        domain = this.domain ?: CommentDomain.NONE.value,
        resource = ga.injuk.commentor.application.port.dto.Resource(
            id = this.resource.id,
...
internal fun CommentPart.convert() = ga.injuk.commentor.domain.model.CommentPart(
...
private fun CommentPartType.convertToDomainCommentPartType() = when (this) {
...
private fun Content.convertToDomainContent() = ga.injuk.commentor.domain.model.Content(
...
private fun ContentType.convertToDomainContentType() = when (this) {
...
private fun MarkType.convertToDomainMarkType() = when (this) {
...
private fun MarkAttrsType?.convertToDomainMarkAttrsType() = when (this) {
